...
export abstract class AbstractAdvancedTradingViewAdapter {
  ...
  abstract getPairTrades(first: number, skip: number, abort: (id: string) => AbortSignal): Promise<void>

  abstract getPairActivity(first: number, skip: number, abort: (id: string) => AbortSignal): Promise<void>
}
...
export class AdvancedTradingViewAdapter {
  ...
  public fetchPairTrades = async (first: number, skip: number) => {
    const promises = Object.values(this._adapters).map(adapter =>
      adapter.getPairTrades(first, skip, this.renewAbortController)
    )
  ...
  public fetchPairActivity = async (first: number, skip: number) => {
    const promises = Object.values(this._adapters).map(adapter =>
      adapter.getPairActivity(first, skip, this.renewAbortController)
    )
