...
const Timeline = () => {
  ...
  return (
    <StyledTimeline width={'full-width'}>
        ...
        <div className="timeline-content" ref={timelineContentRef}>
          <ul className="timeline-milestones">
            {currentMilestoneGroup.map((timelineItem, key) => (
              <Milestone key={key} milestoneIndex={key} milestoneData={timelineItem} />
            ))}
          </ul>
        </div>
        <TimelineControls
      ...
      <BottomRail isCurrentPost={isCurrentPost} currentMilestoneGroupPosition={currentMilestoneGroupPosition} />
    </StyledTimeline>
  )
}
