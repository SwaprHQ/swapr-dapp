...
const CommunityLinks = () => {
  return (
    <StyledCommunityLinks data-aos={'fade-up'} id={'community-links'} width={'full-width'}>
      ...
      <Layout data-aos={'fade-up'} id={'community-links'} width={'main-width'}>
        <span className="pre-header">{CommunityLinksContent.preHeader}</span>
        <h2>{CommunityLinksContent.title}</h2>
        <ul>
          {CommunityLinksContent.links.map((link, key) => (
            <li key={key}>
              ...
              <Button type="dark-outline" label={link.button.label} to={link.button.href} size={'small'} />
            </li>
          ))}
        </ul>
      </Layout>
