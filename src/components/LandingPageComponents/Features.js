      ...
      <section className="top-banner" data-aos={'fade-up'}>
        ...
        <div className="top-banner-logos">
          {FeaturesContent.topBanner.logos.map((logo, index) => (
            <img key={index} src={logo} alt={'Swap, Farm, Bridge & Vote. DeFi unchained.'} />
          ))}
        </div>
      </section>
      <span className="pre-header">{FeaturesContent.preHeader}</span>
      <h2>
        <div className="background-gradient" />
        <span>{FeaturesContent.sectionTitle}</span>
      </h2>
